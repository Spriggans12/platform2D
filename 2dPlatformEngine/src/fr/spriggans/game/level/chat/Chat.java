...
public class Chat implements IChat {
	...
	public void render(Screen screen) {
		...
		if (insideChat) {
			screen.renderRectangle(X_NW_DEBUT_CHAT + screen.xOffs - CHAT_MARGIN + CHAT_BUFER_INSET, Y_NW_DEBUT_CHAT + screen.yOffs
					+ CHAT_HEIGHT + CHAT_MARGIN / 2 - LETTER_LINE_RETURN_HEIGHT * 2 - CHAT_BUFER_INSET - 1,
					MAXIMUM_SEQUENCE_WIDTH_IN_PIXELS + CHAT_MARGIN - CHAT_BUFER_INSET * 2, LETTER_LINE_RETURN_HEIGHT * 2 + 2, 0x33ffffff,
					true);
		}
		...
		if (insideChat) {
			...
			renderMessageBuffer(screen, X_NW_DEBUT_CHAT + CHAT_BUFER_INSET, Y_NW_DEBUT_CHAT + CHAT_HEIGHT + CHAT_MARGIN / 2
					- LETTER_LINE_RETURN_HEIGHT * 2 - CHAT_BUFER_INSET);
		} else {
			for (int i = lastMessages.size() - MESSAGES_NUMBER; i < lastMessages.size(); i++) {
	...
	private void renderMessageBuffer(Screen screen, int startX, int beginY) {
		int beginX = startX;
		final String messageToDisplay = "> " + buffer.toString();
		// Affichage du message.
		...
		for (final String word : words) {
			...
			if (pxlWordSize > MAXIMUM_SEQUENCE_WIDTH_IN_PIXELS) {
				...
				for (final String cutWord : wordInLines) {
					beginY += LETTER_LINE_RETURN_HEIGHT;
					beginX = startX;
					beginX = screen.renderWord(beginX + screen.xOffs, beginY + screen.yOffs, cutWord, 0xFFFFFFFF, LETTER_SPACE_WIDTH,
							LETTER_SPACE_SIZE) - screen.xOffs;
				}
			} else {
				if (beginX + pxlWordSize > MAXIMUM_SEQUENCE_WIDTH_IN_PIXELS) {
					// Trop grand ! On saute une ligne.
				...
				// On affiche le mot.
				beginX = screen.renderWord(beginX + screen.xOffs, beginY + screen.yOffs, word, 0xFFFFFFFF, LETTER_SPACE_WIDTH,
						LETTER_SPACE_SIZE) - screen.xOffs;
			}
		}
	}
	...
	public String tick(String name) {
		...
		if (keyboard.keyDownOnce(KEY_LEAVE_CHAT)) {
			leaveChat();
			keyboard.setLockedInChat(false);
			return null;
		}
	...
	private void handleInput() {
		...
		final boolean shifted = keyboard.keyDown(KeyEvent.VK_SHIFT);

		for (final Key k : keyMap)
	...
	public void addMessage(String autor, String msg) {
		lastMessages.add(new Message(autor, msg));
	}
	...
	private final Key[] keyMap = {//
			...
			new Key(KeyEvent.VK_COMMA, ",", "?"),//
			new Key(KeyEvent.VK_SEMICOLON, ".", ";"),//
			new Key(KeyEvent.VK_COLON, ":", "/"),//
